  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-ink-muted text-sm mt-1">Revisa documentos KYC o verifica usuarios directamente.</p>
      </div>
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all flex items-center gap-2 ${
            subTab === 'kyc'
              ...
              : 'bg-white text-ink-muted border border-line hover:bg-paper-sunk'
          }`}
        >
          <span className="material-symbols-outlined">badge</span> KYC
        </button>
        <button
        ...
        >
          <span className="material-symbols-outlined">verified_user</span> Verificación Directa
        </button>
      </div>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-16">
          ...
          <span className="ml-3 text-ink-muted">Cargando solicitudes...</span>
        </div>
        ...
        <div className="bg-white rounded-xl p-8 shadow-ambient-sm text-center">
          ...
          <h3 className="text-headline-sm text-ink mb-2">
            {filter === 'pending' ? 'No hay solicitudes pendientes' : 'No hay solicitudes ' + filter}
          </h3>
          <p className="text-ink-muted">Todas las solicitudes han sido procesadas</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-4">
          {verifications.map(v => (
            <div key={v.id} onClick={() => openDetailModal(v)}
              className="bg-white rounded-xl p-5 shadow-ambient-sm border border-line/60 hover:shadow-ambient-md transition-all cursor-pointer">
              <div className="flex flex-col lg:flex-row gap-4">
                <div className="flex-1 space-y-3">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="font-semibold text-ink text-sm flex items-center gap-2">
                          ...
                          {v.is_verified && <span className="material-symbols-outlined text-[#15803D] text-xs">check_circle</span>}
                        </h4>
                     ...
                     <div className="space-y-1">
                       <span className="text-xs font-medium text-ink-muted">Estado KYC:</span>
                       ...
                       <span className="text-xs font-medium text-ink-muted">Verificación:</span>
                       <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${
                  ...
                  {v.direccion_apt && (
                    <div className="flex items-center gap-2 text-sm text-ink-muted bg-paper-sunk rounded-lg px-3 py-2">
                      <span className="material-symbols-outlined text-brand-500/60 flex-shrink-0">location_city</span>
                      <span>{v.direccion_apt}{v.barrio ? `, ${v.barrio}` : ''}</span>
                      {v.price && <span className="font-bold text-ink ml-auto">${Number(v.price).toLocaleString('es-CO')}</span>}
                    </div>
                  )}
                  ...
                  {v.admin_notes && (
                    <div className="bg-paper-sunk rounded-lg px-3 py-2 text-xs text-ink-muted">
                ...
                {v.status === 'pending' && (
                  <div className="flex lg:flex-col gap-2 lg:min-w-[140px]">
                    ...
                    >
                      {actionLoading === v.id ? <span className="material-symbols-outlined animate-spin">sync</span> : <span className="material-symbols-outlined">check_circle</span>}
                      Aprobar
                    </button>
                    <button
      ...
      {showRejectModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowRejectModal(false)}>
          <div className="bg-white rounded-xl p-6 shadow-ambient-lg max-w-md w-full animate-scale-in" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-headline-sm text-ink mb-4 flex items-center gap-2 text-danger"><span className="material-symbols-outlined">cancel</span> Rechazar Verificación</h3>
            ...
            <div className="flex gap-3 justify-end">
              <button onClick={confirmReject} disabled={!rejectReason.trim() || actionLoading === rejectTarget?.id}
                ...
                {actionLoading === rejectTarget?.id ? <span className="material-symbols-outlined animate-spin">sync</span> : <span className="material-symbols-outlined">cancel</span>} Rechazar
              </button>
      ...
      {selectedVerification && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={closeDetailModal}>
          <div className="bg-white rounded-2xl shadow-ambient-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto animate-scale-in" onClick={(e) => e.stopPropagation()}>
            {/* Header */}
            <div className="sticky top-0 bg-white border-b border-line px-6 py-4 flex items-center justify-between z-10 rounded-t-2xl">
              <h3 className="text-headline-sm font-bold text-ink flex items-center gap-2">
                <span className="material-symbols-outlined text-brand-500">location_city</span>
                Detalles de la Propiedad
              </h3>
              <button onClick={closeDetailModal} className="w-9 h-9 rounded-lg hover:bg-paper-sunk flex items-center justify-center transition">
                <span className="material-symbols-outlined text-ink-muted">close</span>
              </button>
            </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                <h4 className="text-sm font-semibold text-ink-muted uppercase tracking-wide mb-3 flex items-center gap-2">
                  <span className="material-symbols-outlined text-brand-500">image</span> Fotos del Apartamento
                </h4>
                {detailLoading ? (
                  <div className="flex items-center gap-2 text-sm text-ink-muted py-8 justify-center">
                    <span className="material-symbols-outlined animate-spin">sync</span> Cargando imágenes...
                  </div>
                  ...
                  <div className="bg-paper-sunk rounded-xl py-8 text-center text-sm text-ink-muted">
                    <span className="material-symbols-outlined text-3xl mx-auto mb-2 text-ink-muted">image</span>
              ...
              <div className="mt-6 border-t pt-4">
                ...
                <div>
                  <p className="text-sm font-medium text-ink-muted mb-1">Documento de Identidad (Cédula)</p>
              ...
              <div className="bg-paper-sunk rounded-xl p-4 space-y-2 text-sm">
                <p><span className="font-semibold text-ink-muted">Solicitante:</span> {selectedVerification.user_name} {selectedVerification.user_lastname}</p>
                <p><span className="font-semibold text-ink-muted">Email:</span> {selectedVerification.user_email}</p>
                {selectedVerification.direccion_apt && (
                  <p><span className="font-semibold text-ink-muted">Propiedad:</span> {selectedVerification.direccion_apt}{selectedVerification.barrio ? `, ${selectedVerification.barrio}` : ''}</p>
                )}
                {selectedVerification.admin_notes && (
                  <p><span className="font-semibold text-ink-muted">Notas del admin:</span> {selectedVerification.admin_notes}</p>
                )}
              </div>
              ...
              {selectedVerification.status === 'pending' && (
                <div className="flex gap-3 pt-2">
                  ...
                  >
                    {actionLoading === selectedVerification.id ? <span className="material-symbols-outlined animate-spin">sync</span> : <span className="material-symbols-outlined">check_circle</span>}
      ...
      {usersLoading ? (
        <div className="flex items-center justify-center py-16">
          <span className="material-symbols-outlined animate-spin text-3xl text-brand-500">sync</span>
          <span className="ml-3 text-ink-muted">Cargando arrendadores...</span>
        </div>
      ) : users.length === 0 ? (
        <div className="bg-white rounded-xl p-8 shadow-ambient-sm text-center">
          <span className="material-symbols-outlined text-5xl text-success mx-auto mb-4">verified_user</span>
          <h3 className="text-headline-sm text-ink mb-2">
            {userFilter === 'pendiente' ? 'No hay arrendadores pendientes' : 'No hay arrendadores ' + userFilter}
          </h3>
          <p className="text-ink-muted">Todos los arrendadores han sido procesados</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-4">
          {users.map(u => (
            <div key={u.user_id}
              className="bg-white rounded-xl p-5 shadow-ambient-sm border border-line/60 hover:shadow-ambient-md transition-all">
              <div className="flex flex-col lg:flex-row gap-4">
                <div className="flex-1 space-y-3">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="font-semibold text-ink text-sm flex items-center gap-2">
                          ...
                          {u.estadoVerificacion === 'aprobado' && <span className="material-symbols-outlined text-[#15803D] text-xs">check_circle</span>}
                        </h4>
                ...
                {(u.estadoVerificacion === 'pendiente' || !u.estadoVerificacion) && (
                  <div className="flex lg:flex-col gap-2 lg:min-w-[140px]">
                    <button
                      ...
                      className="flex-1 lg:w-full px-4 py-2.5 rounded-lg bg-[#DCFCE7] text-[#15803D] text-xs font-semibold hover:brightness-95 transition-all flex items-center justify-center gap-1.5 disabled:opacity-50"
                    >
                      {actionLoading === `approve-${u.user_id}` ? <span className="material-symbols-outlined animate-spin">sync</span> : <span className="material-symbols-outlined">check_circle</span>}
                      Aprobar
                    </button>
                    <button
          ...
          <div className="bg-white rounded-xl p-6 shadow-ambient-lg max-w-md w-full animate-scale-in" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-3 justify-end">
              <button onClick={confirmUserReject} disabled={!userRejectReason.trim() || actionLoading === `reject-${userRejectTarget?.user_id}`}
                className="px-5 py-2.5 rounded-lg bg-danger text-white font-semibold text-sm hover:bg-danger-600 transition-all flex items-center gap-2 disabled:opacity-50">
                {actionLoading === `reject-${userRejectTarget?.user_id}` ? <span className="material-symbols-outlined animate-spin">sync</span> : <span className="material-symbols-outlined">cancel</span>} Rechazar
              </button>
