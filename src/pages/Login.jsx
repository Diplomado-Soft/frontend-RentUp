...
function Login() {
  ...
  useEffect(() => {
    document.title = 'Iniciar Sesión | RentUp';
    ...
    <div className="min-h-screen flex flex-col md:flex-row w-full bg-surface">
      ...
      <section className="hidden md:flex relative w-1/2 flex-col justify-center items-center p-8 overflow-hidden bg-brand-navy">
        ...
        <div className="relative z-10 max-w-md text-center text-white">
          ...
          <p className="text-body-md text-white/80 px-8">
      ...
      <section className="flex-1 flex flex-col justify-center items-center p-4 bg-surface-container-lowest min-h-screen" style={{scrollbarGutter: 'stable'}}>
        <div className="w-full max-w-md">
          ...
          <div className="bg-surface-container-lowest p-6">
            ...
            <div className="flex border-b border-surface-variant mb-8">
              ...
              <button onClick={() => navigate('/signup')} className="flex-1 pb-4 text-center border-b-2 border-transparent text-on-surface-variant hover:text-brand-navy transition-colors text-label-md uppercase tracking-wider">
                Registrarse
              </button>
            </div>
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-11 pr-4 py-3 rounded-lg bg-surface-container-low focus:outline-none focus:ring-2 focus:ring-brand-navy/30 transition-all text-on-surface placeholder:text-outline text-body-md"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="••••••••"
                    className="w-full pl-11 pr-11 py-3 rounded-lg bg-surface-container-low focus:outline-none focus:ring-2 focus:ring-brand-navy/30 transition-all text-on-surface placeholder:text-outline text-body-md"
                  />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-outline hover:text-brand-navy transition-colors">
              ...
              <div className="flex items-center justify-between">
                <label className="flex items-center gap-2 cursor-pointer group">
                  <input type="checkbox" className="rounded border-outline-variant text-brand-navy focus:ring-brand-navy/30 h-5 w-5 cursor-pointer" />
                  <span className="text-label-md text-on-surface-variant group-hover:text-on-surface">Recordarme</span>
                </label>
                <span onClick={() => navigate("/forgot-password")} className="text-label-md text-brand-navy hover:underline cursor-pointer transition-all">¿Olvidaste tu contraseña?</span>
              </div>
              ...
              {/* Submit */}
              <button type="submit" className="w-full py-3.5 bg-brand-navy text-white font-bold rounded-lg shadow-md hover:bg-brand-navy/90 active:scale-[0.98] transition-all flex items-center justify-center gap-2 text-label-md uppercase tracking-wider">
                Iniciar Sesión
              </button>
            ...
            <p className="mt-8 text-center text-label-md text-on-surface-variant">
              ...
              <span className="text-brand-navy hover:underline cursor-pointer">Contacta con soporte</span>
            </p>
          </div>
        </div>
        </section>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4" onClick={() => setShowRoleModal(false)}>
          <div className="bg-surface-container-lowest w-full max-w-md rounded-2xl shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className={`p-4 rounded-xl border-2 cursor-pointer transition-all duration-300 ${selectedRole === 'usuario' ? 'border-brand-navy bg-brand-navy/10 shadow-lg' : 'border-surface-container-high hover:border-outline hover:shadow-md'}`} onClick={() => handleRoleSelect('usuario')} disabled={isLoading}>
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${selectedRole === 'usuario' ? 'bg-brand-navy text-white' : 'bg-surface-container-high text-on-surface-variant'}`}>
              ...
              <div className={`p-4 rounded-xl border-2 cursor-pointer transition-all duration-300 ${selectedRole === 'arrendador' ? 'border-brand-navy bg-brand-navy/10 shadow-lg' : 'border-surface-container-high hover:border-outline hover:shadow-md'}`} onClick={() => handleRoleSelect('arrendador')} disabled={isLoading}>
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${selectedRole === 'arrendador' ? 'bg-brand-navy text-white' : 'bg-surface-container-high text-on-surface-variant'}`}>
